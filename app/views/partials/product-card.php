...
<article class="flex h-full flex-col overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200">
    <a href="<?= e(url('produto/' . $product['slug'])) ?>" class="block">
        <img src="<?= e(upload_url('produtos', $product['imagem_principal'])) ?>" alt="<?= e($product['nome']) ?>" class="h-40 w-full object-cover sm:h-52" loading="lazy">
    </a>
    <div class="flex flex-1 flex-col space-y-3 p-3 sm:p-4">
        ...
        <div class="mt-auto flex flex-col gap-3 text-sm sm:flex-row sm:items-center sm:justify-between">
            <a href="<?= e(url('loja/' . $product['loja_slug'])) ?>" class="line-clamp-1 text-slate-500"><?= e($product['nome_loja']) ?></a>
